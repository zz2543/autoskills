...
- `.env`：只放各 provider 的 API key
- `configs/*.yaml`：放 `llm.provider`、`llm.model`、`llm.base_url`、超时、缓存等运行参数

当前代码里已经接好的 provider 枚举是：
...
其余三个 provider 当前只有统一抽象入口，真实 `complete()` 请求还没有实现，直接切过去会触发 `NotImplementedError`。

### 2.1 `.env` 中放什么

把你实际会用到的 key 写进 `.env`。未使用的 provider 可以留空：
...
MINIMAX_BASE_URL=https://api.minimax.io/v1    # 国际区
# MINIMAX_BASE_URL=https://api.minimaxi.com/v1 # 中国区
MINIMAX_MODEL=MiniMax-M2.1
```

然后在 YAML 里确认 provider 为 `minimax`。例如：
...
- 如果你在中国区环境，`base_url` 一般用 `https://api.minimaxi.com/v1`
- 如果你在国际区环境，`base_url` 一般用 `https://api.minimax.io/v1`
- 当前 MiniMax 走的是 OpenAI-compatible `chat/completions` 客户端

### 2.3 ChatGPT / OpenAI 配置方法

OpenAI 在本项目里走同一个 OpenAI-compatible 客户端，但 `provider` 要切成 `openai`，并且必须在 YAML 里显式写 `llm.base_url`。
...
- `ChatGPT` 对应到 API 侧，实际就是把 provider 切到 `openai`
- 当前仓库里的 OpenAI 集成已经能直接发真实请求
- 如果未来切 OpenAI-compatible 的第三方代理，也可以继续复用 `openai` 这条链路，只需要改 `base_url` 和 `model`

### 2.4 Claude / Anthropic 预留配置方法

当前仓库已经预留了 `anthropic` provider 和 `ANTHROPIC_API_KEY` 入口，但 `src/apo_skillsmd/llm/anthropic.py` 仍是占位实现，切过去会抛 `NotImplementedError`。
...
- 有统一入口
- 还不能直接跑真实请求
- 如果要正式启用，需要先实现 `src/apo_skillsmd/llm/anthropic.py`

### 2.5 Gemini 预留配置方法

Gemini 目前也只保留了 provider 切换入口，`src/apo_skillsmd/llm/gemini.py` 还没有真实 function-call / tool-use 适配。
...
- 有 provider 入口
- 还不能直接跑真实请求
- 如果要启用，需要先实现 `src/apo_skillsmd/llm/gemini.py`

### 2.6 Qwen 预留配置方法

Qwen 目前和 Anthropic 一样，结构入口已经在 `factory.py` 里保留，但真实调用还没有补齐。
...
- 有 provider 入口
- 还不能直接跑真实请求
- 如果要启用，需要先实现 `src/apo_skillsmd/llm/qwen.py`

### 2.7 最推荐的切换方式

以后切模型时，优先改 YAML，而不是改代码。

最常见的切换步骤是：

1. 在 `.env` 中填入目标 provider 的 API key
2. 复制一份配置文件，例如从 `configs/default.yaml` 或 `configs/local_mock.yaml` 派生一个新 YAML
3. 修改 `llm.provider`
4. 修改 `llm.model`
5. 如果是 OpenAI-compatible 服务，修改 `llm.base_url`
6. 用 `python scripts/run_experiment.py --config <你的配置>` 启动

例如从 MiniMax 切到 OpenAI：

```yaml
extends: default.yaml
experiment_name: openai_run
llm:
  provider: openai
  model: gpt-4.1-mini
  base_url: https://api.openai.com/v1
...
切换 provider 前，建议先检查这几项：

- `.env` 里是否存在对应的 `*_API_KEY`
- `llm.provider` 是否和 key 对应
- `llm.base_url` 是否和 provider / 区域匹配
- 当前 provider 在本仓库里是否已经实现真实 `complete()` 方法
- `max_tokens` 和 `timeout_sec` 是否适合你当前模型
